...
- **工具按需加载**：全局工具注册表支持按名称获取工具子集，避免工具污染和资源浪费
...
Controller → StartRun
        ↓
    Eino Callbacks (LLM/TOOL/RETRIEVER)
    手动 Span (EMBEDDING/AGENT/CACHE)
    GORM Plugin (DB 慢查询)
        ↓
    异步写入 MySQL
    agent_trace_runs (请求级)
    agent_trace_nodes (节点级)
```
...
```
对话消息累积 → 触发检查 → Summary Agent 压缩 → 保留最近消息 + 历史摘要 → Redis 持久化
```
