...
- `fixed_size`：通用场景，固定字数切分
- `structure_aware`：Markdown 文档，按标题边界切分
- `hierarchical`：知识库默认策略，父子分块，检索子块但送入 LLM 时附上父块

---

### 6. 知识库管理

安全团队积累的内部知识（安全规范、应急预案、历史分析报告）需要系统化管理，才能在 RAG 检索时精准命中。
...
- **多格式解析**：基于文件扩展名自动选择分块策略，`.md/.markdown` 使用 `structure_aware`，其余使用 `hierarchical`，无需用户手动配置
- **并发索引 Worker Pool**：索引任务队列（容量 1000）+ 3 个 Worker 并发执行，DashScope Embedding API 每批 10 条、最大并发 5，充分利用 API 并发能力
- **分块可视化**：Web 界面可查看每个文档的分块列表，包含章节标题、字符数、相似度匹配分数，方便排查检索质量问题
- **查询术语映射（Term Mapping）**：RAG 检索前对查询词做归一化，如将 "Log4Shell" 映射为 "CVE-2021-44228"，提升精确词匹配的召回率
...
- `agent_trace_runs`：每次请求对应一条记录，记录总耗时、Token 消耗、估算美元成本、会话 ID
- `agent_trace_nodes`：每个处理节点（LLM 调用、向量检索、缓存读写）对应一条记录，含父子节点关系、深度、错误信息

**两类节点埋点**

| 节点类型 | 埋点方式 | 记录内容 |
|----------|----------|----------|
| **AGENT** (LLM 调用) | Eino callbacks.Handler | 模型名、输入/输出 Token、成本估算 |
| **CACHE** (缓存) | 手动 span 包装 | 缓存命中状态、操作耗时 |

**检索质量指标**

Retrieval 节点记录 `avg_vector_score`、`max_vector_score`、`doc_count`、`rerank_used`、`avg_rerank_score`，通过追踪界面可以量化每次检索的质量，为调参（TopK、相似度阈值）提供数据支撑。
...
- **KPI 仪表盘**：从 `agent_trace_runs/nodes` 聚合关键指标——平均响应耗时、缓存命中率、平均检索得分、平均 Rerank 得分
- **用户反馈**：聊天界面每条 AI 回复底部支持点赞/点踩，反馈写入 `message_feedbacks` 表，可统计满意度趋势
- **链路关联**：质量评估页面可按 session_id 筛选具体链路，点击查看 Trace 详情，定位低质量回答的根因
- **趋势图**：基于 ECharts 的响应耗时和检索得分趋势图，帮助识别性能退化
