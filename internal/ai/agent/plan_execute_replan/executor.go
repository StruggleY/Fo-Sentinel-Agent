...
func NewExecutor(ctx context.Context) (adk.Agent, error) {
	mcpTool, err := tools.GetLogMcpTool() // 腾讯云 CLS 日志查询（MCP）
	if err != nil {
		return nil, err
	}
	toolList := mcpTool
	toolList = append(toolList, tools.NewPrometheusAlertsQueryTool()) // Prometheus 活跃告警
	toolList = append(toolList, tools.NewQueryInternalDocsTool())     // 内部文档检索
	toolList = append(toolList, tools.NewGetCurrentTimeTool())        // 实时时间戳
