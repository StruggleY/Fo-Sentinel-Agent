// Package intent_recognition intent.go 意图调度器：封装 Router→Executor 流程，管理 cache.SessionMemory 的消息读写。
...
import (
	...
	_ "Fo-Sentinel-Agent/internal/ai/orchestration/intent_recognition/subagents"
...
func NewIntent(ctx context.Context, sessionId string) *Intent {
	...
	if err == nil && (len(recent) > 0 || summary != "") {
		...
		g.Log().Warningf(ctx, "[Intent] 从 Redis 恢复会话状态失败，降级为进程内存，session_id=%s: %v", sessionId, err)
	}
	...
	if output.Content != "" {
		...
		go func() {
			...
			if persistErr := cache.SaveSession(bgCtx, s.sessionId,
				...
				g.Log().Errorf(bgCtx, "[Intent] 保存会话状态到 Redis 失败，session_id=%s: %v", s.sessionId, persistErr)
			}
		}()
	}
