...
// Prometheus 地址从配置 tools.prometheus_url 读取，未配置时回退到 http://127.0.0.1:9090。
func queryPrometheusAlerts(ctx context.Context) (PrometheusAlertsResult, error) {
	baseURL := g.Cfg().MustGet(ctx, "tools.prometheus_url").String()
	if baseURL == "" {
...
// NewQueryMetricsAlertsTool 创建 query_metrics_alerts 工具，查询 Prometheus 活跃告警。
func NewQueryMetricsAlertsTool() tool.InvokableTool {
