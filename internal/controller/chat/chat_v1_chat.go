...
func (c *ControllerV1) Chat(ctx context.Context, req *v1.ChatReq) (res *v1.ChatRes, err error) {
	...
	// 记录当前内存中的对话状态
	g.Log().Debugf(ctx, "[Chat] load memory, session_id=%s, recent_len=%d, has_summary=%t",
	...
	if len(recent) == 0 && summary == "" {
		...
		if r, s, err := cache.GetChatState(ctx, id); err == nil && (len(r) > 0 || s != "") {
			...
			g.Log().Infof(ctx, "[Chat] restored history from redis, session_id=%s, recent_len=%d, has_summary=%t",
				id, len(recent), summary != "")
		} else if err != nil {
			g.Log().Warningf(ctx, "[Chat] failed to load history from redis, session_id=%s, error=%v", id, err)
		}
	}
	...
	//   History - 历史消息列表（包含长期摘要 + 最近消息），注入 Prompt 的 {history} 占位符，
	...
	memory.SetMessages(schema.AssistantMessage(out.Content, nil))

	// 异步持久化到 Redis（不阻塞主链路），服务重启后可恢复历史
	go func() {
		bgCtx := context.Background()
		if err := cache.SetRecentState(bgCtx, id, memory.GetRecentMessages()); err != nil {
			g.Log().Errorf(context.Background(), "failed to persist recent state to redis, session_id=%s, error=%v", id, err)
		} else {
			g.Log().Infof(bgCtx, "[Chat] persisted recent state to redis, session_id=%s, recent_len=%d, has_summary=%t",
